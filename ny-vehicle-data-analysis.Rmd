...
ny_processed <- ny_raw_data %>%
  ...
  filter(
    model_year >= 1970 & model_year <= 2025,
    unladen_weight > 0 & unladen_weight <= 15000,  # Reasonable weight range
...
recent_years <- weight_distribution %>%
...
total_vehicles <- nrow(ny_processed)  # Count individual vehicles
year_range <- range(ny_processed$model_year)

cat("Dataset Summary:\n")
cat("Total passenger vehicles:", scales::comma(total_vehicles), "\n")
cat("Model year range:", year_range[1], "-", year_range[2], "\n")
cat("Weight range:", scales::comma(min(ny_processed$unladen_weight)), "-", 
    scales::comma(max(ny_processed$unladen_weight)), "lbs\n\n")

cat("Recent years (2020+) weight distribution:\n")
for(i in 1:nrow(recent_years)) {
  cat(recent_years$weight_class[i], ":", 
      scales::percent(recent_years$avg_share[i], accuracy = 0.1), "\n")
}
```
